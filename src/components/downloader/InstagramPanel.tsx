...
import { Camera, Download, Trash2, ServerCrash } from 'lucide-react'
import { useDirectDownloader } from '../../utils/useDownloader'
import DownloadProgress from './DownloadProgress'

// The backend endpoint that resolves an Instagram post/reel URL to a direct,
// downloadable media URL.
const RESOLVE_ENDPOINT = '/api/download/instagram'
...
export default function InstagramPanel() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setResolveError('اتصال به سرور استخراج لینک اینستاگرام برقرار نشد. این بخش نیاز به راه‌اندازی بک‌اند دارد.')
    } finally {
      setResolving(false)
    }
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 px-5 py-4 border-b border-[var(--color-border-soft)]">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[#f58529]/15 via-[#dd2a7b]/15 to-[#8134af]/15 text-[#dd2a7b]">
          <Camera size={16} />
        </div>
        <div>
          <h2 className="text-[14px] font-semibold text-[var(--color-text-primary)]">دانلود از اینستاگرام</h2>
          <p className="text-[11px] text-[var(--color-text-muted)]">پست، ریلز یا استوری</p>
        </div>
      </div>
      ...
      <div className="p-5">
        <div className="flex flex-col sm:flex-row gap-2.5">
          <input
            ...
            placeholder="لینک پست یا ریلز اینستاگرام..."
            disabled={busy}
            dir="ltr"
            className="
              ...
              outline-none transition-colors focus:border-[#dd2a7b]/60 disabled:opacity-50
              text-left
            "
          />
          <div className="flex gap-2">
            <button
              ...
              className="
                ...
                bg-gradient-to-r from-[#f58529] via-[#dd2a7b] to-[#8134af] px-4 py-3
